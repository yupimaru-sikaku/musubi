class ContactMailer < ApplicationMailer
    ...
    def user_signup_thanks_mail(current_user)
      @current_user = current_user
      mail(subject: "会員登録ありがとうございます。【むすび】", to: current_user.email)
    end
    
    # 代理店申請した時のサンクスメール
    def company_signup_thanks_mail(current_company, product, registration_fee, shipping_fee)
      tax = 1.1
      @current_company = current_company
      ...
      # 請求金額
      @billing_amount = @shipping_fee.to_i + product.price + registration_fee
